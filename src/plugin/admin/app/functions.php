...
use plugin\admin\utils\JwtAuth;
...
function getCurrentInfo()
{
    ...
    $header = request()->header(config('plugin.admin.project.cross.token_name', 'Authori-zation'));
    if ($header) {
        $token = trim($header);
        if ($token !== 'null' && $token !== '') {
            $jwt = new JwtAuth();
...
{
    $base_dir = config('plugin.admin.project.template', base_path() . '/public/template');
